<script lang="ts">
  // "Install Gemini CLI" prompt — mirror of CodexNotInstalledModal but
  // with Google Gemini CLI's per-OS install commands sourced from
  // github.com/google-gemini/gemini-cli. macOS users get the Homebrew
  // formula path; Linux + Windows install via npm.
  ...
  async function openDocs() {
    try {
      ...
      await openUrl('https://github.com/google-gemini/gemini-cli');
    } catch (_) {}
  }
</script>
...
<Modal bind:show title="Gemini CLI Not Found" width="440px">
  <div class="cn-body">
    ...
    {#if platform === 'mac'}
      {@render cmdBlock('Homebrew', 'brew install gemini-cli')}
      {@render cmdBlock('npm', 'npm install -g @google/gemini-cli')}
...
{#snippet cmdBlock(label: string, cmd: string)}
...
<style>
  ...
  .cn-code {
    ...
    padding: 1px 5px; color: var(--t1);
  }
  .cn-section { display: flex; flex-direction: column; gap: 5px; }
  .cn-label {
    font-size: 11px; font-weight: 600; color: var(--t3);
    text-transform: uppercase; letter-spacing: 0.06em; font-family: var(--ui);
  }
  .cn-cmd-block {
  ...
  .cn-after {
    ...
    padding: 9px 12px; background: var(--c);
  ...
  .cn-btn-docs {
    ...
    cursor: default; transition: border-color 0.1s, color 0.1s;
  }
